...
for (let i=0 ; i < NewArr.length ; i++){
  ...
  ObjList.push(`<li>${NewArr[i].join(': ')}</li>`);
  }
return ObjList;
}
...
Write a function named addValues that, given an array of numbers as input, uses reduce to add
...
const addValues = (arr) => {
  // Solution code here...
  let NewArr =arr.reduce((AddVal,value)=>{
    AddVal += value;
    return AddVal ;
    },0);
    return NewArr;
...
const addPurchases = (arr) => {
  // Solution code here...
  let NewArr =arr.reduce((AddPur,value)=>{
    AddPur += value.purchasePrice;
    return AddPur ;
    },0);
    return NewArr;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let LenArr = arr.reduce((count, value, index) => {
    count = + index;
    return count + 1;
  })
  return LenArr;
...
const returnNames = (arr) => {
  // Solution code here...
  let ArrNames = arr.reduce((ReName, Property) => {
    ReName.push(Property.name);
    return ReName;
  }, []);
  return ArrNames;
};
...
const reversedString = (str) => {
  ...
  let revStr = NewArr.reduce((Reversed, value) => {
